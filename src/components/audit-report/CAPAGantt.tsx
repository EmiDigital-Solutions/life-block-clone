  ...
  return (
    <section className="scroll-mt-20 py-12 space-y-8">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[13px] font-semibold tracking-[0.15em] uppercase text-muted-foreground">CAPA Timeline</span>
      </div>
      ...
      <div className="flex items-stretch gap-px bg-border">
        {[
          ...
          <div key={s.label} className="flex-1 p-4 bg-card">
            <span className="text-[12px] uppercase tracking-[0.12em] font-semibold text-muted-foreground">{s.label}</span>
      ...
      <div className="p-8 bg-card shadow-sm">
        <div className="flex justify-between mb-4 text-[13px] font-mono text-muted-foreground">
          <span>Apr 8</span>
          <span>Apr 22</span>
          <span>May 6</span>
          <span>May 20</span>
              ...
              <div key={item.id} className="flex items-center gap-4 py-3 ">
                ...
                <div className="w-[280px] shrink-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[13px] font-mono font-bold text-muted-foreground">{item.id}</span>
                    <span className={`text-[12px] font-mono px-1.5 py-0.5 ${statusStyles[item.status] || 'bg-muted text-muted-foreground'}`}>{item.status}</span>
                    {item.severity === 'major' && (
                      <span className="text-[11px] px-1.5 py-0.5 font-bold uppercase bg-destructive/10 text-destructive">Major</span>
                    )}
                  </div>
                  <p className="text-[15px] text-foreground truncate">{item.title}</p>
                  <div className="flex items-center gap-1.5 mt-1">
                    <User className="w-3 h-3 text-muted-foreground" />
                    <span className="text-[13px] text-muted-foreground">{item.owner}</span>
                  </div>
                </div>
