...
export default function NCRCard({ ncr, compact, onAssign, onAction }: NCRCardProps) {
  ...
  return (
    <div className={cn("rounded-xl border p-5 transition-all bg-white", sev.border)}>
      ...
      <div className="rounded-lg bg-[#F5F5F5] border border-[#E5E7EB] p-3 mb-4">
      ...
      <div className="flex items-center gap-4 mb-4">
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-dashed border-[#C0C0C0] text-[12px] text-[#7B8E80] hover:border-[#0A7FA5] hover:text-[#0A7FA5] transition-colors"
        >
          <User className="w-3.5 h-3.5" />
          {ncr.owner || 'Assign owner'}
        </button>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-dashed border-[#C0C0C0] text-[12px] text-[#7B8E80] hover:border-[#0A7FA5] hover:text-[#0A7FA5] transition-colors">
      ...
      <div className="flex items-center gap-2">
        {ncr.severity === 'major' ? (
          <>
            <button onClick={() => onAction?.(ncr.id, 'reject')} className="px-3 py-1.5 rounded-lg bg-[#AD3D3D]/10 text-[#AD3D3D] text-[12px] font-medium hover:bg-[#AD3D3D]/20 transition-colors">Reject</button>
            <button onClick={() => onAction?.(ncr.id, 'escalate')} className="px-3 py-1.5 rounded-lg bg-[#0A7FA5]/10 text-[#0A7FA5] text-[12px] font-medium hover:bg-[#0A7FA5]/20 transition-colors">Escalate</button>
            <button onClick={() => onAction?.(ncr.id, 'accept-deviation')} className="px-3 py-1.5 rounded-lg bg-[#F5F5F5] text-[#7B8E80] text-[12px] font-medium hover:bg-[#E5E7EB] transition-colors">Accept w/ deviation</button>
          </>
        ) : (
          <>
            <button onClick={() => onAction?.(ncr.id, 'accept')} className="px-3 py-1.5 rounded-lg bg-[#6EA996]/10 text-[#6EA996] text-[12px] font-medium hover:bg-[#6EA996]/20 transition-colors">Accept</button>
            <button onClick={() => onAction?.(ncr.id, 'rework')} className="px-3 py-1.5 rounded-lg bg-[#E39B5C]/10 text-[#E39B5C] text-[12px] font-medium hover:bg-[#E39B5C]/20 transition-colors">Rework</button>
            <button onClick={() => onAction?.(ncr.id, 'reject')} className="px-3 py-1.5 rounded-lg bg-[#AD3D3D]/10 text-[#AD3D3D] text-[12px] font-medium hover:bg-[#AD3D3D]/20 transition-colors">Reject</button>
            <button onClick={() => onAction?.(ncr.id, 'escalate')} className="px-3 py-1.5 rounded-lg bg-[#F5F5F5] text-[#7B8E80] text-[12px] font-medium hover:bg-[#E5E7EB] transition-colors">Escalate</button>
          </>
        )}
      </div>
