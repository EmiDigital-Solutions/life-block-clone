  ...
  return (
    <aside className={cn(
      ...
      <div className="text-right px-4 pt-3 pb-1">
        <span className="text-[13px]" style={{ color: 'hsl(0,0%,50%)' }}>Last saved <span className="text-primary font-medium">2 min ago</span> · Auto-save on</span>
      </div>

      {/* Header */}
      <div className="px-5 pb-4" style={{ borderBottom: '1px solid hsl(0,0%,76%)' }}>
        <div className="flex items-center justify-between">
          <span className="text-[12px] font-bold tracking-[0.15em] uppercase" style={{ color: 'hsl(0,0%,45%)' }}>Section Inspector</span>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-5 py-4" style={{ borderBottom: '1px solid hsl(0,0%,76%)' }}>
          <span className="text-[12px] font-bold tracking-[0.15em] uppercase mb-3 block" style={{ color: 'hsl(0,0%,45%)' }}>Properties</span>
        ...
        {stationNCRs.length > 0 && (
          <div className="px-5 py-4" style={{ borderBottom: '1px solid hsl(0,0%,76%)' }}>
            <span className="text-[12px] font-bold tracking-[0.15em] uppercase mb-3 block" style={{ color: 'hsl(0,0%,45%)' }}>Linked NCRs</span>
            <div className="space-y-2">
              {stationNCRs.map(ncr => (
                <div key={ncr.id} className="flex items-center gap-3 py-1.5">
                  ...
                  <span className="text-[15px] font-medium text-foreground flex-1">{ncr.id} · {ncr.title.toLowerCase().split(' ').slice(0, 2).join(' ')}</span>
                  <span className="text-[14px] capitalize" style={{ color: 'hsl(0,0%,50%)' }}>{ncr.severity}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {(station.evidenceCount.photos > 0 || station.evidenceCount.measurements > 0 || station.evidenceCount.videos > 0) && (
          <div className="px-5 py-4" style={{ borderBottom: '1px solid hsl(0,0%,76%)' }}>
            <span className="text-[12px] font-bold tracking-[0.15em] uppercase mb-3 block" style={{ color: 'hsl(0,0%,45%)' }}>Linked Evidence</span>
            <div className="grid grid-cols-2 gap-2">
              {Array.from({ length: Math.min(station.evidenceCount.photos, 4) }).map((_, i) => (
                <div key={i} className="aspect-square flex items-end p-2" style={{ background: 'linear-gradient(135deg, hsl(207,33%,75%,0.3), hsl(143,18%,75%,0.2))', border: '1px solid hsl(0,0%,76%)' }}>
                  <span className="text-[12px] font-mono px-1.5 py-0.5 bg-foreground/60 text-white">
              ...
              {(station.evidenceCount.photos + station.evidenceCount.measurements + station.evidenceCount.videos) > 5 && (
                <div className="aspect-square flex items-center justify-center" style={{ background: 'hsl(0,0%,88%)', border: '1px solid hsl(0,0%,76%)' }}>
                  <span className="text-[14px] font-medium" style={{ color: 'hsl(0,0%,50%)' }}>
                    +{station.evidenceCount.photos + station.evidenceCount.measurements + station.evidenceCount.videos - 5}
                  </span>
                </div>
              )}
        ...
        <div className="px-5 py-4">
          <span className="text-[12px] font-bold tracking-[0.15em] uppercase mb-3 block" style={{ color: 'hsl(0,0%,45%)' }}>Audit Timeline</span>
          <div className="space-y-0">
            {timelineEvents.map((evt, i) => (
              <div key={i} className="flex gap-3 py-2">
                ...
                <div>
                  <span className="text-[13px] block" style={{ color: 'hsl(0,0%,50%)' }}>{evt.time}</span>
                  <span className="text-[15px] text-foreground font-medium">{evt.label}</span>
                </div>
              </div>
            ))}
          </div>
...
function PropertyRow({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-[15px]" style={{ color: 'hsl(0,0%,50%)' }}>{label}</span>
      <span className="text-[15px] font-medium tabular-nums" style={{ color: valueColor || 'hsl(var(--foreground))' }}>{value}</span>
    </div>
  );
}