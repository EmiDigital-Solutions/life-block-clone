...
const TimeEfficiencyChart = () => {
  ...
  const phases = [
    { name: 'Auditor Search', traditional: 14, scanpro: 1, visualScanpro: 0.8, colorClass: 'bg-blue-muted' },
    { name: 'Preparation', traditional: 7, scanpro: 2, visualScanpro: 0.8, colorClass: 'bg-accent' },
    { name: 'On-site Audit', traditional: 3, scanpro: 2, visualScanpro: 0.8, colorClass: 'bg-gray-medium' },
    { name: 'Report', traditional: 10, scanpro: 0.5, visualScanpro: 0.5, colorClass: 'bg-blue-muted' },
  ];
    ...
    <div ref={ref} className="w-full">
      <div className="space-y-6 sm:space-y-10 mb-6 sm:mb-10">
            ...
            {phases.map((phase, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative group cursor-pointer ${phase.colorClass}`}
                >
        ...
        <div>
          <div className="flex items-center justify-between mb-3 sm:mb-5">
            <h4 className="text-xs sm:text-sm md:text-base font-bold text-gray-900 flex items-center gap-2 sm:gap-3">
              <Zap className="w-4 h-4 sm:w-5 sm:h-5 drop-shadow-md text-accent" strokeWidth={2.5} />
          ...
          <div className="relative flex items-center gap-2 sm:gap-4">
            ...
            >
              <span className="text-lg sm:text-xl md:text-2xl font-black drop-shadow-md text-accent">{totalScanPro} days</span>
            </motion.div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-3 sm:gap-x-6 gap-y-2 sm:gap-y-3 mb-6 sm:mb-8">
        {phases.map((phase, idx) => (
          <div key={idx} className="flex items-center gap-2 sm:gap-3">
            <div className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded flex-shrink-0 ${phase.colorClass}`} />
      ...
      {/* Summary */}
      <div className="p-4 sm:p-6 md:p-8 bg-secondary/10 rounded-xl sm:rounded-2xl">
